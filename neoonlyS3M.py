...
def define_s3m():
    ...
    neo_s3m = pd.concat(dfs, ignore_index=True)
    
    neo_s3m["a"] = neo_s3m["q"] / (1 - neo_s3m["e"])
    print("The final df is done.")
    return neo_s3m

def s3m_array(neo_s3m):
...
#n_H, n_a, n_e, n_i = 352,342,325,322
    points = np.vstack([neo_s3m["H"], neo_s3m["a"], neo_s3m["e"], neo_s3m["i"]]).T
    edges = [
    ...
    return neo_s3m, array4D, H_center, a_center, e_center, i_center
