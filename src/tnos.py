...
def define_s3m():
    ...
    tnos_s3m = pd.concat(dfs, ignore_index=True)
    
    tnos_s3m["a"] = tnos_s3m["q"] / (1 - tnos_s3m["e"])
    print("The final df is done.")
    return tnos_s3m

def s3m_array(tnos_s3m):
# defining the min/max values for a,e,i,H
    a_min = tnos_s3m["a"].min()
    a_max = tnos_s3m["a"].max()
    e_min = tnos_s3m["e"].min()
    e_max = tnos_s3m["e"].max()
    i_min = tnos_s3m["i"].min()
    i_max = tnos_s3m["i"].max()
    H_min = tnos_s3m["H"].min()
    H_max = tnos_s3m["H"].max()

    n_H, n_a, n_e, n_i = 52,42,25,22
#n_H, n_a, n_e, n_i = 152,142,125,122
#n_H, n_a, n_e, n_i = 352,342,325,322
    points = np.vstack([tnos_s3m["H"], tnos_s3m["a"], tnos_s3m["e"], tnos_s3m["i"]]).T
    edges = [
    ...
    return tnos_s3m, array4D, H_center, a_center, e_center, i_center
