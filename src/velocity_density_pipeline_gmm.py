"""
velocity_density_pipeline_gmm.py
==================================

GMM-NEO variant of velocity_density_pipeline_hybrid.py.
Training data: same hybrid_elements.parquet via hybrid_loader.py.

Identical to the hybrid pipeline in all respects EXCEPT:
  - NEO cloning uses a Gaussian Mixture Model (GMM) instead of conditional K|M.
  - MBA / TNO / Trojan cloning is unchanged (still conditional K|M).

The GMM is trained on the visible source NEOs per mag bin, then samples
clone_factor x N_visible orbital elements. This gives broader orbital
diversity than K|M (which only repeats each parent orbit) while keeping
efficiency comparable. K|M fallback is applied automatically if GMM fails.

GMM feature vector (cloning_test_ZI_v2.ipynb, advisor-approved):
  log(a), q, sin/cos(i), sin/cos(node), sin/cos(argperi), sin/cos(M_obs)
H is sampled empirically from source NEO distribution (not a GMM feature).

DO NOT MODIFY velocity_density_pipeline_hybrid.py — this is a parallel clone.
Maps written to prob_maps_gmm/ (not prob_maps_hybrid/).
...
DEFAULT_POPULATION_SETTINGS = {
    ...
    "NEO":     {"s3m_pop": "neo",    "clone_factor": 80, "scatter_size": 4, "scatter_alpha": 0.08},
    "TNO":     {"s3m_pop": "tno",    "clone_factor": 10, "scatter_size": 6, "scatter_alpha": 0.10},
    "Trojans": {"s3m_pop": "trojan", "clone_factor": 5,  "scatter_size": 6, "scatter_alpha": 0.10},
}
...
def build_cloned_maps_for_center_magbin(
    ...
    for pop_name, info in clone_sources.items():
        ...
        if info.get("use_conditional_cloner", True):
            ...
            if pop_name == "NEO":
                # GMM cloner: train on visible source NEOs, sample f*N clones
                # Falls back to K|M if GMM fails for any reason.
                ...
                try:
                    ...
                    print(f"  GMM clones sampled: {len(gmm_clone_df):,}  "
                          f"acceptance: {gmm_diag['acceptance_fraction']:.3f}  "
                          f"converged: {gmm_diag['converged']}")
                    clone_visible_df = build_visible_subset_dataframe(
                        gmm_clone_df,
                ...
                if not gmm_success:
                    print("  Falling back to K|M for NEO.")
                    ...
                    print("  K|M fallback visible points:", len(clone_visible_df))
            else:
                # K|M cloner for MBA, TNO, Trojans — unchanged
                clone_visible_df = clone_population_conditional_K_from_M_with_skycut(
                    df=df_cloner_input, clone_factor=f, obstime_str=obstime_str,
                    scorer=info["scorer"], max_sep_deg=max_sep_deg,
                    center_mode="custom_ecliptic",
