"""
velocity_density_pipeline_hybrid.py
====================================

Hybrid-catalog variant of velocity_density_pipeline_fast.py.
Training data: hybrid_elements.parquet (14.4M objects from hybrid.h5 —
90.8% S3M synthetic + 9.2% real MPCORB), loaded via hybrid_loader.py.

Identical to the fast pipeline in all respects except:
  1. Imports hybrid_loader instead of s3m_loader
  2. Clone factors tuned for hybrid population sizes (MBA=1, NEO=80, TNO=10, Trojans=5)

DO NOT MODIFY velocity_density_pipeline_fast.py — this is a parallel clone.
Maps written to prob_maps_hybrid/ (not prob_maps/).
