#!/bin/bash
#SBATCH --job-name=sorcha_gen_maps
...
mkdir -p "$WORKDIR/logs" "$WORKDIR/prob_maps"
...
OUTPUT="${WORKDIR}/prob_maps/prob_maps_${DATE}_antisun.npz"
echo "output=${OUTPUT}"

"$PY" "$WORKDIR/neomod/pipeline/sorcha_gen_map.py" \
    --obstime    "${OBSTIME}"    \
