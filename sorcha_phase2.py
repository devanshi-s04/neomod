...
NEOMOD_SRC = ROOT / "neomod" / "src"
ADAM_STUB = ROOT / "adam_core_stub"
if ADAM_STUB.exists():
...
def score_vdp_frame(df: pd.DataFrame, prob_maps_dir: Path, cache: dict[str, vdp.ProbMapSet]) -> pd.DataFrame:
    df = df.copy()
    df["P_NEO_vdp"] = np.nan
    df["vlam"] = np.nan
    df["vbeta"] = np.nan
